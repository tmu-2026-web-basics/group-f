...
const collectionNumber = pageMatch ? Number(pageMatch[1]) : 1;
...
if (!document.querySelector(".footer")) {
  detail.insertAdjacentHTML("afterend", `
    <footer class="footer">
      <p class="footer-text">created by groupF</p>
    </footer>
  `);
}
...
function renderCollectionCarousel(interiors) {
  const otherCollections = interiors
    ...
    .filter((item) => item.index !== collectionIndex);
...
async function loadCollection() {
  try {
    ...
    const item = interiors[collectionIndex];

    if (!item) {
      throw new Error(`コレクション${collectionNumber}のデータが見つかりません。`);
    }
    ...
    document.title =
      `${item.product_name} | Collection ${String(collectionNumber).padStart(2, "0")}`;
